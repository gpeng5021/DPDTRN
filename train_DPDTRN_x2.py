...
import math
import numpy as np
from dataset.dataset_x2 import DatasetFromFolder
...
parser.add_argument("--scale", type=int, default=2, help="training batch size")
...
def main():
    ...
    train_set = DatasetFromFolder(opt.train_file[0], opt.train_file[1], opt.train_file[2])
...
def train(training_data_loader, optimizer, model, criterion, epoch, tb_writer, criterion_lap):
    ...
    for iteration, batch in tqdm(enumerate(training_data_loader, 1), desc='train dataset',
        ...
        input_img, HR_label, _ = Variable(batch[0]), Variable(batch[1], requires_grad=False), Variable(batch[2],
                                                                                                       requires_grad=False)
        ...
        x2, textures = model(input_img)

        textures = torch.tanh(textures)
        loss = criterion(x2, HR_label) + 0.1 * criterion_lap(x2, HR_label, textures)
