...
parser.add_argument("--scale", type=int, default=8, help="training batch size")
...
parser.add_argument("--train_file", default=['./data/Text330/train/total_crop',
                                             ...
                                             './data/Text330/train/total_crop_lr'
                                             ],
                    type=list)

Model_Name = 'DPDTRN_x8'
...
def train(training_data_loader, optimizer, model, criterion, epoch, tb_writer, criterion_lap):
    ...
    for iteration, batch in tqdm(enumerate(training_data_loader, 1), desc='train dataset',
        ...
        input_img, x2_label, HR_label, xx_label = Variable(batch[0]), Variable(batch[1], requires_grad=False), Variable(
        ...
        if opt.cuda:
            ...
            xx_label = xx_label.cuda()

        x8, textures = model(input_img)

        textures = torch.tanh(textures)
        loss = criterion(x8, HR_label) + 0.1 * criterion_lap(x8, HR_label, textures)
