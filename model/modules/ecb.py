...
class ECB(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            y = self.conv1x1_3x3(x) + self.conv1x1_lpl(x)
            if self.with_idt:
                y += x
        else:
...
if __name__ == '__main__':
    ...
    # conv = SeqConv3x3('conv1x1-conv3x3', 1, 1, 1)
    # y0 = conv(x)
    # print('aa', y0.shape)
    ...
    # x = torch.randn(1, 1, 5, 5)
    # # conv = SeqConv3x3('conv1x1-sobelx', 3, 3, 2)
    # conv = SeqConv3x3('conv1x1-sobelx', 1, 1, 1)
    # y0 = conv(x)
    # RK, RB = conv.rep_params()
    # y1 = F.conv2d(input=x, weight=RK, bias=RB, stride=1, padding=1)
    # print('11', y0 - y1)
    ...
    print(x[1][1][1][1])
    ecb = ECB(inp_planes=64, out_planes=64, depth_multiplier=2.0, act_type='prelu', with_idt=True)
    y0 = ecb(x)
    print(y0[1][1][1][1])