...
class _Usmnet(nn.Module):
    def __init__(self, scale=8, in_channels=64, out_channels=64, nf=64, nb_Usm_module=1, gc=32):
        super(_Usmnet, self).__init__()

        self.Usm_upconv = nn.ConvTranspose2d(in_channels=64, out_channels=64, kernel_size=4, stride=2, padding=1,
                                             bias=False)
        self.Usm_upconv8x = nn.ConvTranspose2d(in_channels=64, out_channels=64, kernel_size=10, stride=8, padding=1,
                                               bias=False)
...
class _Usm_module(nn.Module):
    ...
    def forward(self, x, usm_lr):
        ...
        fea = x + trunk
        fea_usm = self.usm_conv(fea - usm_lr)
...
class Usmsrn(nn.Module):
    def __init__(self, in_c=1, out_c=1, scale=8):
